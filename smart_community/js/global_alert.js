document.addEventListener('DOMContentLoaded', () => {
    ...
    function showToast(title, message) {
        ...
        if (audio) {
            audio.currentTime = 0;
            audio.play().catch(e => console.log("Audio blocked by browser, user needs to interact with page first", e));
        }
    ...
    async function monitorSystem() {
        try {
            ...
            if (alerts.length === 0) {
                lastAlertId = 0;
                localStorage.setItem('last_alert_id', 0);
            } else {
                const maxDbId = Math.max(...alerts.map(a => parseInt(a.id)));
                if (maxDbId < parseInt(lastAlertId)) {
                    lastAlertId = 0;
                    localStorage.setItem('last_alert_id', 0);
                }
            }
