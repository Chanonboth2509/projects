...
$sos_count = 0;
// 🌟 จุดที่แก้: สั่งให้นับหมดเลย ทั้งเคสที่เป็น pending, เป็นค่าว่าง, หรือเป็น NULL
$sql_sos = "SELECT COUNT(*) as count FROM alerts WHERE type = 'SOS' AND (status = 'pending' OR status IS NULL OR status = '')";
$res_sos = $conn->query($sql_sos);
if ($res_sos) {
    $sos_count = $res_sos->fetch_assoc()['count'];
}
...
// ส่งตัวเลขกลับไปให้หน้า Dashboard
echo json_encode([
