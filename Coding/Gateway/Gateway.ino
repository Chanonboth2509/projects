...
String baseUrl = "http://192.168.2.37/smart_community/api/"; 
...
unsigned long lastDownlink = 0, lastGatewayHb = 0; 
...
void setNewMessage(String type, String msg) {
  ...
  if (textPixelWidth > 120) { needsScrolling = true; scrollX = 128; } 
  else { needsScrolling = false; scrollX = 128; }
  updateScreen();
}

void handleXamppFail() {
  if (xampp_active) { xampp_active = false; lastStatus = "Server Offline"; updateScreen(); }
}
...
void setup() {
  ...
  while(WiFi.status() != WL_CONNECTED && wifi_attempts < 20) { 
    delay(500); lastStatus = "WiFi Connecting..."; updateScreen(); wifi_attempts++;
  }
  if(WiFi.status() == WL_CONNECTED) { lastStatus = "WiFi OK"; } 
  else { lastStatus = "LoRa Only Mode"; xampp_active = false; }
  
  setNewMessage("สถานะระบบ:", "- พร้อมรับข้อมูล -"); delay(1000);

  SPI.begin(SCK, MISO, MOSI, SS); LoRa.setPins(SS, RST, DI0);
  if (!LoRa.begin(BAND)) { Serial.println("LoRa Fail"); while(1); }
  
  LoRa.setSyncWord(0x12); LoRa.setSpreadingFactor(7); LoRa.setSignalBandwidth(125E3); 
  LoRa.setCodingRate4(5); LoRa.enableCrc();
}

void loop() {
  handleLed(); 
  if (needsScrolling && millis() - lastScrollTime > scrollSpeed) {
    scrollX--; if (scrollX < -textPixelWidth) scrollX = 128; 
    updateScreen(); lastScrollTime = millis();
  }
  ...
  if (packetSize) {
    ...
    if (p1 != -1 && p2 != -1) {
      ...
      if(WiFi.status() == WL_CONNECTED) {
        lastStatus = "Web Syncing.."; updateScreen(); 
        HTTPClient http; http.setTimeout(1000); 
        String url = baseUrl + "insert_alert.php?node_id=" + n_id + "&battery=" + n_bat + "&msg=" + urlEncode(n_msg) + "&rssi=" + String(rssi);
        http.begin(url);
        int code = http.GET();
        if (code > 0) { lastStatus = "Saved OK"; xampp_active = true; } 
        else { handleXamppFail(); }
        http.end(); updateScreen();
      } else {
        lastStatus = "LoRa RX (No Web)"; updateScreen();
      }
    }
  }
  ...
  if (isDisplayingAlert && (now - lastMsgTime > DISPLAY_TIMEOUT)) {
      isDisplayingAlert = false; setNewMessage("สถานะระบบ:", "- พร้อมรับข้อมูล -");
      if(WiFi.status() == WL_CONNECTED) { lastStatus = "System Ready"; } 
      else { lastStatus = "LoRa Only Mode"; }
      updateScreen();
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
      if (now - lastGatewayHb > 60000 || lastGatewayHb == 0) {
        ...
        int code = http.GET();
        if (code > 0) { xampp_active = true; if(lastStatus == "Server Offline") { lastStatus = "System Ready"; updateScreen(); } } 
        else { handleXamppFail(); }
        http.end();
      }
      
      if (xampp_active) {
          if (now - lastDownlink > 4000) { checkDownlink(); lastDownlink = now; }
      }
  }
}
...
void checkDownlink() {
  ...
  http.begin(baseUrl + "broadcast.txt?t=" + String(millis())); 
  ...
  if (httpCode == 200) {
    ...
    if (payload.length() == 0) {
        lastWebMsg = ""; http.end(); return;
    }

    if (payload != lastWebMsg) {
      lastWebMsg = payload;
      digitalWrite(LED_PIN, HIGH); 

      int separatorIdx = payload.indexOf('|');
      if (separatorIdx != -1) {
          String targets = payload.substring(0, separatorIdx);
          String msg = payload.substring(separatorIdx + 1);

          int start = 0;
          int sentCount = 0;
          while(start < targets.length()) {
              int commaIdx = targets.indexOf(',', start);
              if (commaIdx == -1) commaIdx = targets.length();
              
              String rawNode = targets.substring(start, commaIdx);
              String cleanNode = "";
              for(int i=0; i<rawNode.length(); i++) {
                  char c = rawNode.charAt(i);
                  if(isAlphaNumeric(c) || c == '-') cleanNode += c;
              }

              if (cleanNode.indexOf("NODE-") != -1) {
                  LoRa.beginPacket(); 
                  LoRa.print("GATEWAY|100|" + msg + "|" + cleanNode); 
                  LoRa.endPacket(); 
                  delay(600); 
                  sentCount++;
              }
          ...
          lastMsgTime = millis();
          isDisplayingAlert = true;
          setNewMessage("กระจายข่าว:", "ส่งหา " + String(sentCount) + " เครื่อง");
      }
