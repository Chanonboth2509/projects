...
int rawDebug = 0; 
const char* menus[] = { "ขอความช่วยเหลือ", "ไฟไหม้ !", "มีผู้บุกรุก/โจร", "เจ็บป่วยฉุกเฉิน", "อุบัติเหตุ/รถชน", "น้ำท่วม", "สัตว์มีพิษ" };
...
void updateBattery() {
  ...
  else { if (stablePct < 0) stablePct = currentPct; 
  else if (currentPct < stablePct) stablePct = (stablePct * 0.95) + (currentPct * 0.05); }
}
...
void updDispI() {
  if(!scnOn) return; dispI.clear(); dispI.setFont(ArialMT_Plain_10); 
...
void updDispE() {
  ...
  if(mode == LOCKED) {
    ...
    dispE.setCursor(80, 0); if(rawDebug >= 1050) dispE.print("CHG"); else dispE.print(String((int)stablePct)+"%");
    ...
    if (senderName != "") { dispE.setCursor(0, 10); dispE.print("FROM: " + senderName); } 
    else { dispE.setCursor(0, 10); dispE.print("MESSAGE:"); }
...
void OnRxDone(uint8_t *pl, uint16_t sz, int16_t rs, int8_t sn) {
  // 🌟 แก้ไข: ป้องกัน Buffer Overflow ลิมิตความยาวข้อความไม่ให้บอร์ด Node แฮงก์
  char b[256]; 
  int len = (sz < 255) ? sz : 255;
  memcpy(b, pl, len); 
  b[len]='\0'; 
  String in(b);
  ...
  String target = "";
  if (p3 != -1 && p3 > p2) { 
      target = in.substring(p3 + 1);
      target.trim();
  } else if (p2 != -1) {
      target = in.substring(p2 + 1);
      target.trim();
  }

  // 🌟 บล็อกประกาศที่ไม่ได้ส่งหาเรา หรือไม่ได้ส่งหา ALL
  if (senderID.indexOf("GATEWAY") != -1) {
      if (target != "ALL" && target != devID) {
          Radio.Rx(0); 
          return; 
      }
  }

  // Relay SOS (ยอมให้ผ่านไปทำ Relay ตามปกติ)
  ...
  triggerLed(5); wake(); msg = content;
  senderName = (senderID=="NODE-001") ? "233/2" : (senderID=="NODE-012") ? "234/1" : senderID;
  txtX = 128; mode = IDLE;
...
void setup() {
  ...
  String hello = devID + "|" + String(rawDebug >= 1050 ? 101 : (int)stablePct) + "|POWERON โหนดพร้อมทำงาน";
...
void loop() {
  ...
  if(now - lastHb > HB_INTV) {
    ...
    String pkt = devID + "|" + String(rawDebug >= 1050 ? 101 : (int)stablePct) + "|STATUS|Via:" + target;
    ...
    if(digitalRead(ROT_SW) == LOW) {
      ...
      while(digitalRead(ROT_SW) == LOW) {
        ...
        if (mode != SEND) {
          ...
          if(holdTime > 5000 && !sosTriggered) {
            ...
            String pkt = devID + "|" + String(rawDebug >= 1050 ? 101 : (int)stablePct) + "|SOS ขอความช่วยเหลือด่วน|Direct";
      ...
      if(!sosTriggered && (millis() - pStart > 50)) {
         if(mode == LOCKED) { beep(1, 20); updDispE(); }
            ...
            String pkt = devID + "|" + String(rawDebug >= 1050 ? 101 : (int)stablePct) + "|" + String(menus[menuIdx]) + "|Direct";
  ...
  if(scnOn) {
      if (mode == SEND) { scnTmr = millis(); }
      unsigned long timeoutLimit = (msg != "" && msg != "ส่งข้อมูลแล้ว" && msg != "ส่งไม่สำเร็จ" && msg != "กำลังส่ง...") ? 30000 : 15000;
      if ((millis() - scnTmr > timeoutLimit) && mode != SEND) { 
