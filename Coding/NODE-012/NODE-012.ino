...
void OnRxDone(uint8_t *pl, uint16_t sz, int16_t rs, int8_t sn) {
  ...
  triggerLed(5); wake(); msg = content; senderName = (senderID=="NODE-001") ? "233/2" : (senderID=="NODE-012") ? "234/1" : senderID;
...
void loop() {
  ...
  if(digitalRead(ROT_SW) == LOW) {
    delay(30); if(digitalRead(ROT_SW) == LOW) {
      ...
      if(!sosTriggered && (millis() - pStart > 50)) {
         ...
         // 🌟 จุดที่ 2: ลบเงื่อนไข Deep Sleep ออกไปแล้ว ให้กดปุ่มแล้วส่งข้อความตามเมนูที่เลือกทันที
         else if(mode == MENU) {
  ...
  if(scnOn) {
      ...
      if ((millis() - scnTmr > timeoutLimit) && mode != SEND) { 
          dispI.displayOff(); 
          dispE.oled_command(SH110X_DISPLAYOFF); 
          scnOn = false; 
          mode = LOCKED; 
          unlockProgress = 0; 
          msg = ""; 
